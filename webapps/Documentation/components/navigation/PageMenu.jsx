...
class PageMenu extends React.Component {
    ...
    return navigation.map(
      section => {
        ...
        return (
          <Section
            key={ section.slug }
            isActive={ page.getSection({ path: activePath }) === page.getSection({ path: sectionPath }) }
            title={ section.title }
            path={ sectionPath }
            onClick={ onNavigate }
          />
  ...
  static renderSecondLevel ({ activeVersion, activePath, expandedPath, navigation, onNavigate, onPageClick }) {
    ...
    const expandedSectionSlug = page.getSection({ path: expandedPath });
    const expandedChapterSlug = page.getChapter({ path: expandedPath });
